...
class DatabaseOperations(BaseDatabaseOperations):
    ...
    def date_extract_sql(self, lookup_type, field_name):
        ...
        """
        lookup_type_conversions = { "year":"'yyyy'", "month":"'m'", "day":"'d'", "week_day":"'w'" }
        #if lookup_type == 'week_day':
        #    return "DATEPART(dw, %s)" % field_name
        #else:
        return "DATEPART(%s, %s)" % (lookup_type_conversions[lookup_type], field_name)
    ...
    def date_trunc_sql(self, lookup_type, field_name):
        ...
        """
        if lookup_type == 'year':
            return "CDATE(CSTR(DATEPART('yyyy', %s)) + '/01/01')" % field_name
        if lookup_type == 'month':
            return "CDATE(CSTR(DATEPART('yyyy', %s)) + '/' + CSTR(DATEPART('m', %s)) + '/01')" % (field_name, field_name)
        if lookup_type == 'day':
            return "%s" % field_name
    ...
    def last_insert_id(self, cursor, table_name, pk_name):
        ...
        cursor.execute("SELECT MAX(CLNG(%s)) FROM %s" % (pk_name, table_name))
