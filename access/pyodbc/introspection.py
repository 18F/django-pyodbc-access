...
WHERE a.TABLE_NAME = %s AND a.CONSTRAINT_TYPE = 'FOREIGN KEY'"""
		...
		# sadly, my access DB doesn't have any relations saved between tables.
		# but if it did, I think it would involve the hidden table "MSysRelationships" in access.
		# you can reveal this table in the interface by going to:
		# Tools-Options-View-Show System Objects-OK
		# and then do
		# Tools-Security-User and Group Permissions -- Select MSysRelationships from object names --
...
#WHERE a.TABLE_NAME = %s AND (CONSTRAINT_TYPE = 'PRIMARY KEY' OR CONSTRAINT_TYPE = 'UNIQUE')"""
        ...
        field_names = [item[3] for item in cursor.columns(table=table_name)]
        indexes = {}
		# first, get a list of the indexed columns
        ...
        for field_name in field_names:
            try:
			    ...
			    # then put a new entry into the indexes dictionary with false for everything (as the column is not an index)
                indexes.update({field_name: {"primary_key":False, "unique":False, "db_index":False}})
        return indexes
